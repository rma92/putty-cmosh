...
* mc arrow keys: DECCKM/application cursor tracking is implemented but mc navigation still does not work. A diagnostic was added (commit after b26197a): mosh.c now logs "Mosh terminal: application cursor keys (DECCKM on/off)" to the PuTTY Event Log whenever the cmosh terminal model changes cursor mode. Run mc and check the Event Log to determine if DECCKM is being set. If it is never set, mc's smkx does not include `\033[?1h` (likely a TERM variable issue — the mosh-server may not be passing `--terminal-type xterm-256color`). If DECCKM is set but arrows still fail, the issue is downstream of key generation.
